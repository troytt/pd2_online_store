...
from skills import ELE_SKILLS_NAMES, SKILLS_TREE_NAMES, SKILLS_TREE_OFFSETS, SKILL_NAMES
from utils import (
    ReverseBitReader,
    calc_bits_to_align,
    obj_to_dict,
...
class Item(object):
  ...
  @property
  def item_img(self):
    return self._items_data.get_item_img(self.code)
  
  @property
  def item_size(self):
    return self._items_data.get_item_size(self.code)

  @property
  def stash_x(self):
    if self.location_id == self.LOC_STORED and self.panel_id == self.PANEL_STASH:
      return self.pos_x
    else:
      return -1
  ...
  @property
  def inventory_x(self):
    if self.location_id == self.LOC_STORED and self.panel_id == self.PANEL_INVENTORY:
      return self.pos_x
    if self.location_id == self.LOC_EQUIPPED:
          if self.equipped_id == self.EQUIP_HEAD:
            return 2
          elif self.equipped_id == self.EQUIP_NECK:
            return 4
          elif self.equipped_id == self.EQUIP_TORSO:
            return 2
          elif self.equipped_id == self.EQUIP_HAND_RIGHT:
            return 4
          elif self.equipped_id == self.EQUIP_HAND_LEFT:
            return 0
          elif self.equipped_id == self.EQUIP_FINGER_RIGHT:
            return 3
          elif self.equipped_id == self.EQUIP_FINGER_LEFT:
            return 2
          elif self.equipped_id == self.EQUIP_WAIST:
            return 2
          elif self.equipped_id == self.EQUIP_FEET:
            return 4
          elif self.equipped_id == self.EQUIP_HANDS:
            return 0
    return -1
  
  @property
  def inventory_y(self):
    if self.location_id == self.LOC_STORED and self.panel_id == self.PANEL_INVENTORY:
      return self.pos_y + 9
    if self.location_id == self.LOC_EQUIPPED:
          if self.equipped_id == self.EQUIP_HEAD:
  ...
  def _parse_simple(self):
    ...
    if self.is_ear:
      ...
      self.code = ''.join(chr(self._reader.read(8)) for _ in range(4)).rstrip()
  ...
  def _parse_magic_attrs(self):
    ...
    while True:
      ...
      if not attr_dict:
        raise ItemParseError(f'Unknown magic attribute id: {magic_attr_id}')
      bias = attr_dict.get('bias', 0)
      values = [self._reader.read(bits) - bias for bits in attr_dict['bits']]
      ...
      elif magic_attr_id in (204, 359):
        values[1] = SKILL_NAMES.get(values[1])
      elif magic_attr_id in (151, 97, 107):
...
# https://github.com/nokka/d2-armory/tree/master
