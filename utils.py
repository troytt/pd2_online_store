...
def print_binary_string(data, size):
    fmt_string = '{:0%db}' % size
...
class ReverseBitReader(object):
    ...
    def read(self, bits):
        ...
        while self._bits_read < bits:
            byte_read = self._reader.read(1)
            if byte_read == b'':
                raise ItemParseError('Read to end of file')
            byte = _reverse_bits(
                int.from_bytes(byte_read, byteorder='little'), 8
            )
            self._data <<= 8
            self._data |= byte
            self._bits_read += 8
        ...
        if _debug:
            fmt_string = '{:0%db}' % bits
            print('[end position:0x{:X}'.format(self._reader.tell()-1), '+%d'%self._bits_read, 'data:',fmt_string.format(result), ']')
        return result
...
if __name__ == '__main__':
  reader = open('test/item/item45.d2i', 'rb')
  rbit_reader = ReverseBitReader(reader)
  for i in range(10):
    rbit = rbit_reader.read(8)