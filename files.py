...
from classes import CLASS_NAMES, CLS_NECROMANCER
from errors import D2SFileParseError, ItemParseError, StashFileParseError
from item import Item
from items_storage import ItemsDataStorage
from skills import SKILL_NAMES, SKILL_OFFSETS
from utils import (
    ReverseBitReader,
    _BytesJSONEncoder,
    int_from_bbytes,
  ...
  def _parse_items(self, skip_items_header=False):
    ...
    if skip_items_header:
      ...
      if items_header != self._ITEMS_HEADER:
        raise ItemParseError(f'Invalid items header: 0x{items_header:04X}')
    ...
    while items_count:
      ...
      if item.location_id == Item.LOC_SOCKETED:
        ...
        if socketed_item.itype == Item.T_WEAPON:
          socket_attrs = self._items_data.get_weapon_sock_attrs(item.code)
        elif socketed_item.itype == Item.T_ARMOR:
          socket_attrs = self._items_data.get_armor_sock_attrs(item.code)
        elif socketed_item.itype == Item.T_SHIELD:
          socket_attrs = self._items_data.get_shield_sock_attrs(item.code)
        ...
        for attr in socket_attrs:
          attr_name = self._items_data.get_magic_attr(attr['id'])['name']
          socketed_item.magic_attrs.append(attr_name.format(*attr['values']))
        socketed_item.socketed_items.append(item)
...
class D2SFile(_D2File):
  ...
  def __init__(self, d2s_path):
    ...
    # special bit for pd2
    self._reader.read(3)
    # Also skip old pd2 save file
    if self.last_played <= 1682438400:
      return
  ...
  def _parse_skills(self):
    ...
    if skill_header != self._SKILLS_HEADER:
      raise D2SFileParseError(f'Invalid skill header id: {skill_header:02X}')
    skills = {}
  ...
  def _parse_stash_pages(self):
    ...
    for page in range(self.page_count):
      ...
      if stash_header != self._STASH_HEADER:
        raise StashFileParseError(f'Invalid stash header: 0x{stash_header:08X}')
      flags = 0
      name = None
...
class D2XFileSplitter():
  """Split PlugY personal stash file (.d2x)."""
  ...
  def __init__(self, stash_file_path, no_stash=False):
    """Initializes an instance.
    ...
    # {page number => page}
    self.stash = {}
    stash_bytes = self._parse_header(stash_file_path)
    page_no = 0
    item_count = 0
    for page_byte in stash_bytes.split(self._STASH_HEADER):
      if len(page_byte) <= 1:
        continue
      #print('Split page ', page_no)
      if no_stash and page_no != self.page_count-1:
        page_no += 1
        continue
      self.stash[page_no] = []
      items = self._parse_page(self._STASH_HEADER+page_byte)
      for item in items:
        if len(item) <= 1:
          continue
        #item_file_path = Path(
        #    output_file_path).joinpath('item%d.d2i'%item_count)
        #open(item_file_path, 'wb').write(self._ITEM_HEADER+item)
        item_count += 1
        item_byte_io = BytesIO(self._ITEM_HEADER+item)
        
        try:
          item = Item(item_byte_io)
          self.stash[page_no].append(item)
          if item.to_dict()['code'].startswith('r01') or item.to_dict()['code'].startswith('r09'):
            print(item_count)
        except:
          pass
  ...
  def _parse_page(self, page_byte):
    stash_header = page_byte[0:2]
    if stash_header != self._STASH_HEADER:
      raise StashFileParseError(f'Invalid stash header: 0x{stash_header}')
    flags = 0
    name = None
    pos = 2
    data = page_byte[2:4]
    if data != self._ITEM_HEADER:
      _flags = int_from_bbytes(page_byte[2:6])
      flags = dict(
          is_shared=bool((_flags >> 24) & 0b1),
          is_index=bool((_flags >> 16) & 0b1),
          is_main_index=bool((_flags >> 8) & 0b1),
    ...
    while True:
      ...
      name = name + byte
    #print('name:', name, '\nflags:', flags)
    item_count = int_from_lbytes(page_byte[pos+2:pos+4])
    #print('item_count:', item_count)
    items = page_byte[pos+4:].split(self._ITEM_HEADER)
    return items

  def _parse_header(self, stash_file_path):
    """Parses the header.
    ...
    f = open(stash_file_path, 'rb')
    header = f.read(4)
    if header != self._HEADER:
      raise StashFileParseError(f'Invalid header id: 0x{header}')
    self.version = f.read(2)
    if self.version != self._VERSION:
      raise StashFileParseError(f'Invalid version: {self.version}')
    f.read(4)
    self.page_count = int_from_lbytes(f.read(4))
    # print("Page count: ", self.page_count)
    pages = f.read()
    f.close()
    return pages
...
if __name__ == '__main__':
  from pathlib import Path
  data_dir = Path(__file__).parent.joinpath('test/save/')
  #item_dir = Path(__file__).parent.joinpath('test/item/')
  for file_path in data_dir.iterdir():
    if not str(file_path).endswith('d2x'):
      continue
    print(file_path)
    splitter = D2XFileSplitter(file_path, True)
    p = next(iter(splitter.stash.values()))
    for item in p:
        if item.location_id == 1:
            print(item, item.a(), item.b())
    input()
    #for page in splitter.stash:
    #  if (len(splitter.stash[page]) <= 0):
    #    continue
    #  for item in splitter.stash[page]:
    #    print(item.item_img)
