...
from utils import stripped_string_concat
...
class ItemsDataStorage(object):
    ...
    _ITEMS_DATA_DIR = 'items_data_pd2'
    ...
    def get_corruption(self, corruption_id):
        """Gets the corruption attribute by id.

        :param corruption_id: Corruption magic attribute identifier
        :type corruption_id: int
        :return: Corruption name
        :rtype: str
        """
        return self._corruption.get(corruption_id)
    ...
    def get_item_img(self, code):
        """Gets the image path of an item.
        ...
        :rtype: str
        """
        if code in self._item_img:
            item = self._item_img.get(code)
            if 'type' in item and 'sub_type' in item and 'img_name' in item:
              return '/'.join([self._item_img.get(code)['type'], self._item_img.get(code)['sub_type'], self._item_img.get(code)['img_name']])
        return ''

    def get_item_size(self, code):
        """Gets the size of this item.

        :param code: item code
        :type code: str
        :return: Items size
        :rtype: x, y size pair e.g. [1,2]
        """
        if code in self._item_img:
            return (self._item_img.get(code)['size_x'], self._item_img.get(code)['size_y'])
        else:
            return (1, 1)
